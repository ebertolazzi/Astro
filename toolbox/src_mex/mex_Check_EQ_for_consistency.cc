...
namespace AstroLib {
  ...
  mexFunction( int nlhs, mxArray       *plhs[],
    ...
    try {
      UTILS_ASSERT0( nrhs == 6, "Check_EQ_for_consistency: Expected 6 arguments" );
      UTILS_ASSERT0( nlhs == 1, "Check_EQ_for_consistency: Expected 1 outputs" );

      // Get values of the scalar inputs
      // Check for the proper type of argument
      real_type p = Utils::mex_get_scalar_value( arg_in_0, "Lambert first argument (p) must be a scalar\n" );
      real_type f = Utils::mex_get_scalar_value( arg_in_1, "Lambert 2rd argument (f) must be a scalar\n" );
      real_type g = Utils::mex_get_scalar_value( arg_in_2, "Lambert 3rd argument (g) must be a scalar\n" );
      real_type h = Utils::mex_get_scalar_value( arg_in_3, "Lambert 4rd argument (h) must be a scalar\n" );
      real_type k = Utils::mex_get_scalar_value( arg_in_4, "Lambert 5rd argument (k) must be a scalar\n" );
      real_type L = Utils::mex_get_scalar_value( arg_in_5, "Lambert 6rd argument (K) must be a scalar\n" );
