...
namespace Utils
{
  ...
  */
  ...
  template <typename Real>
  void
  divide( Poly<Real> const & p, Poly<Real> const & q, Poly<Real> & M, Poly<Real> & R )
  {
    ...
    else
    {
      ...
      UTILS_ASSERT0( !is_zero( lcQ ), "Poly::divide(p,q,M,R), leading coefficient of q(x) is 0!" );
  ...
  */
  ...
  template <typename Real>
  void
  GCD( Poly<Real> const & p, Poly<Real> const & q, Poly<Real> & g, Real epsi )
  {
    if ( q.order() > 0 )
    {
      Poly<Real> M, R;
