...
namespace Utils {
  ...
  AlgoHNewton<Real>::set_max_iterations( Integer mit ) {
    UTILS_ASSERT(
      mit > 0,
      "AlgoHNewton::set_max_iterations({}) argument must be >0\n", mit
    );
    m_max_iteration = mit;
  }
  ...
  */
  ...
  AlgoHNewton<Real>::eval() {
    ...
    while ( ++m_iteration_count < m_max_iteration ) {
      ...
      else if ( x > 1-m_kappa ) x = 1-m_kappa;
      m_d  = m_a + x * m_ba;
      m_fd = evaluate( m_d );
      ...
      if ( m_fc * m_fd < 0 ) {
        ...
        if ( m_fa * m_fc > 0 ) { m_a = m_d; m_fa = m_fd; abs_fa = abs(m_fa); }
        else                   { m_b = m_c; m_fb = m_fc; abs_fb = abs(m_fb); }
      }
    }
